 ...
 * my mwk snippets to a sequence file, but with stop word analysis.
 *
 */
public class WritePointsToSequenceFile2 {
	...
	public static void main(final String[] args) throws Exception {
		...
		doClustering();
		System.err
				.println("SRIDHAR MahoutTermClusterMwkSnpt.main() - Once you've figured out how to get the document vectors from term finding, feed them to doClustering() instead of static points.");
	}
	...
	private static void doTermFinding() throws Exception {
		...
		System.err
				.println("MahoutTermFinder.main() - Reading TFIDF Vectors (this will take a while)");
		...
		for (String mwkDirectory : tfidf.keySet()) {
			...
			Map<String, Double> allTermsWithScoresForMwkDirMap;
			{
				...
				// System.err.println("MahoutTermFinder.convert() " +
				// terms1.size());
				int i = 0;
				...
				for (Element scoreElement : tfidf2.get().all()) {
					...
					if (!terms1.containsKey(id)) {
						throw new RuntimeException("Couldn't find key " + id
								+ ", only found " + terms1.keySet());
					}
					...
					// System.err.println("MahoutTermFinder.transform() term = "
					// + term);
					j++;
				}
		...
		System.err
				.println("MahoutTermFinderMwkSnpt.filter() - Discarding terms with low scores.");
		...
		for (String file : scores.keySet()) {
			...
			// System.err.println("SRIDHAR MahoutTermFinderMwkSnpt.filter() - "
			// + file + "
		...
		System.err
				.println("MahoutTermFinderMwkSnpt.main() - printing terms that satisfy the minimum score.");
		for (String filename : filter.keySet()) {
			...
			List<Entry<String, Double>> sortedEntries = new ArrayList<Entry<String, Double>>(
					scoresForDocument.entrySet()).subList(0,
					Math.min(20, scoresForDocument.size()));
	...
	private static Path writeToSequenceFile(Configuration configuration,
		...
		for (String path : mwkSnippetCategoryDirs) {
			Text cateogoryDir = new Text(Paths.get(path).getFileName()
					.toString());
			DirectoryStream<java.nio.file.Path> stream = Files
					.newDirectoryStream(Paths.get(path));
			System.err.println("SRIDHAR MahoutTermFinderMwkSnpt.main() - "
					+ cateogoryDir + "::" + Paths.get(path).toFile());
			try {
				for (java.nio.file.Path fileInPath : stream) {
					if (Files.isDirectory(fileInPath)) {
						...
						if (fileInPath.toFile().exists()) {
							...
							writer.append(
									cateogoryDir,
									new Text(FileUtils.readFileToString(Paths
											.get(fileInPath.toUri()).toFile())));
